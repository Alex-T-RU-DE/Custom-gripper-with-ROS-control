         ...
         youbot_gripper::grip_service::Response &res)
{
   ...
   if(data_check(req.pos) && safety_check())
   {
    switch(static_cast<int>(req.pos / 1000))
             {    case 0:
                         SERVO.WritePos(1, (930 - map(req.pos, 0, 100, 0, 340)), 0x01FF);
                         SERVO.WritePos(12,(90  + map(req.pos, 0, 100, 0, 340)), 0x01FF);
                         int lastServoLoad = SERVO.Get_Load(1);
                         const int posError  = 30;  //assuming the data might have uncertainty
                         const int loadError = 50; //assuming the data might have uncertainty
                         const int graspingLoad = 1000; 
                         //checking whether object was grasped or not
                         while(abs(SERVO.Get_Pos(1) - (930 - map(req.pos, 0, 100, 0, 340))) > posError)
                             {
                              delay(500);
                              if((abs(lastServoLoad - SERVO.Get_Load(1))<=loadError) && (SERVO.Get_Load(1) > graspingLoad))
                                  break;
                               lastServoLoad = SERVO.Get_Load(1);                            
                             }
                          delay(200);
                          if(SERVO.Get_Load(1)>graspingLoad)
                            {
